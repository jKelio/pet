...
**Team**:
A group the Tenant fields, identified by an **Altersklasse** (an integer 7–21, rendered "U{n}" in the UI, e.g. U16) and a free-text **name** (e.g. "Lions"). The combination `(Altersklasse, name)` is unique per Tenant — two teams may share the same age class if their names differ. A Tenant has one or more Teams. All Members can track and view any Team — there is no per-Team assignment. Contrast with an [[External Team]], which the Tenant tracks but does not field.
_Avoid_: Group, squad.
_Note_: A member selects a registered Team from a dropdown during Tracking Setup; a new session is bound to the Team by ID at selection time. Legacy sessions created before Altersklasse was introduced remain bound by free-text name match (see ADR 0014).
